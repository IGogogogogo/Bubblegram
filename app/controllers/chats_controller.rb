class ChatsController < ApplicationController
  ...
  def index
    ...
    @chats = Chat.between(current_user, @users).includes(:sender, :recipient).order(updated_at: :desc)
  end
  ...
  def show
    ...
    redis.lrem("#{current_user.id}_chat_notice",0,"#{@chat.id}")
